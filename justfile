...
#
# `build` needs cargo, blueprint-compiler and the libvirt, gvnc, spice-glib, libusb and
# VTE headers; `install` only copies what is already in target/release, so the two can run
# on different machines sharing this directory.
#
#   just build
